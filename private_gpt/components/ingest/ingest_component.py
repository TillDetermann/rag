...
class BaseIngestComponent(abc.ABC):
    ...
    transformations_per_file: dict[Path, dict[str, list[TransformComponent]]]) -> list[Document]:
        pass
...
class SimpleIngestComponent(BaseIngestComponentWithIndex):
    ...
    transformations_per_file: dict[Path, dict[str, list[TransformComponent]]],
        ...
        for file_name, file_data in files:
            ...
            transformations_per_doc_type = transformations_per_file[file_data]
            documents = IngestionHelper.transform_file_into_documents(
                file_name, reader, file_data
            )
            saved_documents.extend(self._save_docs(documents, transformations_per_doc_type))
    ...
    transformations_per_doc_type: dict[str, list[TransformComponent]],
        ...
        if len(transformations_per_doc_type) == 1:
            ...
            for doc in documents:
                doc_type = doc.metadata.get("doc_type", default_key)
                if doc_type in transformations_per_doc_type:
                    grouped[doc_type].append(doc)
                else:
                    grouped[default_key].append(doc)
